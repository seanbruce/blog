...
> Disclaimer: As a hint, if you already know what the built-in hook `useSyncExternalStore` do and what problem it solved, You should probably pass this blog.
...
I was excited when first time I saw this pattern when reading source code of [react-query](https://tanstack.com/query/v4) and [zustand](https://github.com/pmndrs/zustand). I thought that I just found a smart way of doing
performance optimization. It couldn't be more wrong and I realized that later when I figure out the missing part when I wrote my own 'external store'.
...
- In `handleUpdate` callback, if the data change detected, It should trigger a re-render.
